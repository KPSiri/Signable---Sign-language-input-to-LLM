...
# --- INITIAL SETUP ---
st.set_page_config(page_title="Sign to LLM", layout="wide")

# Custom CSS for a professional "Dark Mode" feel
st.markdown("""
    <style>
...
# --- CLAUDE LLM INTEGRATION (FIXED 404) ---
def call_claude(api_key, gloss_words):
    client = anthropic.Anthropic(api_key=api_key)
    gloss_str = " ".join(gloss_words).upper()
    
    # Modern model ID for 2026 stability
    current_model = "claude-sonnet-4-20250514" 

    format_resp = client.messages.create(
        model=current_model,
        max_tokens=100,
        messages=[{"role": "user", "content": f"Convert these ASL glosses into a natural English interrogative sentence: [{gloss_str}]. Reply with ONLY the sentence."}]
    )
...
#st.title("Signable",anchor=True)
st.markdown("<h1 style='text-align: center;'>Signable</h1>", unsafe_allow_html=True)
...
with col_cam:
    with st.container(border=True):
        ...
        # Display detected glosses in a clean badge style
...
with col_chat:
    with st.container(border=True):
        ...
        if st.button("Send to Claude ⚡", type="primary", use_container_width=True) and st.session_state.gloss_list:
            with st.spinner("Interpreting signs..."):
                try:
